...
export default class TsParserConfig {
  ...
  public static createConfigFile(dir?:string) {
    let fullName = configFileName;

    if('string' == typeof dir) {
      fullName = path.join(dir, configFileName);
    }

    try {
      fs.writeFileSync(fullName, configFileData, { encoding: 'utf8', flag: 'wx' });
